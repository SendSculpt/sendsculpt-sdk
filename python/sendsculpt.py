...
class SendSculptClient:
    ...
    def __init__(self, api_key: str, base_url: str = "https://api.sendsculpt.com/api/v1"):
        """
        Initialize the SendSculptClient.
        
        :param api_key: Your SendSculpt API key.
        :param base_url: The base URL for the SendSculpt API.
        """
    ...
    def send_email(
        ...
        if template_id and (body_html or body_text):
            raise ValueError("template_id and body_html/body_text cannot be provided together.")
        ...
        if attachments is not None:
            ...
            for att in attachments:
                ...
                # If they provided raw bytes
                    ...
                    fmt_att["content"] = base64.b64encode(att["content_bytes"]).decode('utf-8')
                # If they provided a file path
                elif "file_path" in att:
                    if not os.path.exists(att["file_path"]):
                        raise FileNotFoundError(f"Attachment file not found: {att['file_path']}")
                    with open(att["file_path"], "rb") as f:
                        fmt_att["content"] = base64.b64encode(f.read()).decode('utf-8')
                else:
                    raise ValueError("Attachment must specify 'content' (base64 string), 'content_bytes' (bytes), or 'file_path' (string).")
