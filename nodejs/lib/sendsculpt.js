...
class SendSculptClient {
     ...
     * @param {string} [baseUrl] - Optional custom base URL (useful for local testing).
     */
    constructor(apiKey) {
        if (!apiKey) {
    ...
    async sendEmail(options) {
        ...
        try {
            ...
            throw new Error(`SendSculpt API Error: ${body ? body.message : "Unknown error"}`);
        } catch (error) {
            if (error.response) {
                const body = error.response.data;
                throw new Error(
                    `SendSculpt API Error [${error.response.status}]: ${body ? body.message || JSON.stringify(body) : error.message}`
                );
            }
            throw error;
        }
